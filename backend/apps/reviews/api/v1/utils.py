...
def _call_text_summarizer(prompt_messages):
    ...
    """
    endpoint = getattr(settings, "AZURE_INFERENCE_ENDPOINT", "https://models.github.ai/inference")
    ...
    client = ChatCompletionsClient(endpoint=endpoint, credential=AzureKeyCredential(api_token))
    resp = client.complete(messages=prompt_messages, model=model, temperature=0.2, top_p=1.0)

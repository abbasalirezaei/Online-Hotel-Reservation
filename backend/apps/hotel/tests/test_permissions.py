...
class TestIsHotelOwnerOrReadOnly:
    ...
    def test_read_only_access(self):
        ...
        assert self.permission.has_object_permission(request, None, hotel) is True
    ...
    def test_non_owner_write_access_denied(self):
        ...
        request = self.factory.put("/api/v1/hotels/")
        request.user = UserFactory()
        assert self.permission.has_object_permission(request, None, hotel) is False
    ...
    def test_room_owner_write_access(self):
        ...
        assert self.permission.has_object_permission(request, None, room) is True

    def test_non_room_owner_write_access_denied(self):
        room = RoomFactory()
        request = self.factory.put("/api/v1/rooms/")
        request.user = UserFactory()
        assert self.permission.has_object_permission(request, None, room) is False

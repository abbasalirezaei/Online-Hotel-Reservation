...
def test_review_triggers_notification(user_factory, hotel_factory):
    ...
    review = Review.objects.create(
        user=reviewer, hotel=hotel, rating=4, comment="Great stay!"
    )
...
def test_reply_triggers_notification(user_factory, hotel_factory):
    ...
    parent_review = Review.objects.create(
        user=original_reviewer, hotel=hotel, rating=5, comment="Loved it!"
    )

    reply = Review.objects.create(
        user=replier, hotel=hotel, parent=parent_review, rating=4, comment="Thanks!"
    )

...
def test_send_custom_notification_success(hotel_owner_factory, user_factory):
    ...
    send_custom_notification(
        target_user.id, "Special offer for you!", "info", "/offers"
    )

    assert Notification.objects.filter(
        user=target_user, message="Special offer for you!"
    ).exists()
...
def test_send_global_notification_success(hotel_owner_factory):
    ...
    assert Notification.objects.filter(
        is_global=True, message="Big discount for all users!"
    ).exists()
...
def test_anonymous_cannot_send_custom_notification(user_factory):
    ...
    payload = {"user_id": target_user.id, "message": "Hello!"}
